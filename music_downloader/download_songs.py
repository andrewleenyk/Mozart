...
This script downloads songs from a text file using yt-dlp.
It parses the text file by genre and downloads each song as MP3.
"""
    ...
    def download_song(self, song_info: str, genre: str) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                '--output', f'{genre_dir}/%(title)s.%(ext)s',  # Output format
                '--no-playlist',  # Don't download playlists
                '--max-downloads', '1',  # Only download first result
                '--add-metadata',  # Add metadata
                '--parse-metadata', 'title:%(title)s',  # Parse title
                '--parse-metadata', 'artist:%(uploader)s',  # Parse artist
                '--parse-metadata', 'genre:%(genre)s',  # Parse genre
                '--no-check-certificates',  # Skip certificate verification
                '--ignore-errors',  # Continue on errors
                f'ytsearch1:{search_query}'  # Search YouTube
            ]
...
def main():
    ...
    downloader = MusicDownloader(songs_file, output_dir)
