#!/usr/bin/env python3
"""
Working Music Downloader Script

A reliable version that downloads songs from a text file using yt-dlp.
"""
...
class WorkingMusicDownloader:
    def __init__(self, songs_file: str = "songs.txt", output_dir: str = "downloads"):
        """
        Initialize the working music downloader.
    ...
    def download_song(self, song_info: str, genre: str) -> bool:
        ...
        try:
            ...
            # Clean up song info for search - remove parentheses and extra info
            clean_song = re.sub(r'\s*\([^)]*\)', '', song_info)  # Remove (or Johnny Cash) etc.
            search_query = clean_song.replace(' - ', ' ')
            
            print(f"🎵 Downloading: {song_info}")
            print(f"   Genre: {genre}")
            print(f"   Search: {search_query}")
            
            # Use the proven ytsearch1 format without quotes
            search_url = f"ytsearch1:{search_query}"
            
            # yt-dlp command with proven options
            cmd = [
                'yt-dlp',
                '--extract-audio',
                '--audio-format', 'mp3',
            ...
            # Run the download command with longer timeout
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
            
            # yt-dlp returns 0 for success, 101 for max downloads reached (also success)
...
def main():
    """Main function to run the working music downloader."""
    print("🎵 Working Music Downloader")
    print("=" * 40)
    ...
    downloader = WorkingMusicDownloader(songs_file, output_dir)
